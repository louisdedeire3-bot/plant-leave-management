...
type PortalRole = "employee" | "supervisor" | "manager" | "admin";
    ...
    async (token: string) => {
      ...
      try {
        const [employeeResult, leaveResult, overtimeResult] = await Promise.all([
          supabase.rpc("portal_employees", { p_token: token }),
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto grid max-w-[1680px] grid-cols-1 gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:px-8">
        ...
        <main className="min-w-0">
          ...
          {view === "manager" && (profile.role === "manager" || profile.role === "admin") && (
            <ApprovalDashboard
                ...
                <Fragment key={departmentName}>
                  ...
                  {departmentEmployees.map((employee, employeeIndex) => (
                    <tr key={employee.id} className={employeeIndex % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      <td className="sticky left-0 z-10 border-b border-r border-slate-300 bg-inherit px-4 py-2"><div className="flex items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center border border-slate-500 bg-slate-800 font-mono text-xs font-black text-white">{initials(employee)}</span><div className="min-w-0"><p className="truncate font-black uppercase text-slate-950">{employeeName(employee)}</p><p className="font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">{employee.employeeCode}</p></div></div></td>
                      {rangeDates.map((date) => {
...
function EmployeeCell({ employee }: { employee: Employee }) { return <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center border border-slate-700 bg-slate-900 text-xs font-black text-white">{initials(employee)}</span><div><p className="font-black text-slate-950">{employeeName(employee)}</p><p className="font-mono text-xs text-slate-500">{employee.employeeCode}</p></div></div>; }
