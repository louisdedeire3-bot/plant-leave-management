...
interface ProductionConsumableMovement {
  id: number;
  consumableId: string;
...
interface ProductionConsumablesResponse {
  items: ProductionConsumable[];
  movements: ProductionConsumableMovement[];
}
...
interface ProductionBootstrap {
  ...
  consumables: ProductionConsumable[];
  consumableMovements: ProductionConsumableMovement[];
}
...
export function ProductionFactoryModule({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [productionResult, consumablesResult] = await Promise.all([
        supabase.rpc("portal_production_bootstrap", {
          p_token: sessionToken,
          p_limit: 300,
        }),
        supabase.rpc("portal_production_consumables", {
          p_token: sessionToken,
          p_limit: 500,
        }),
      ]);
      ...
      const productionData = (productionResult.data ??
        emptyBootstrap) as ProductionBootstrap;
      const consumablesData = (consumablesResult.data ?? {
        items: [],
        movements: [],
      }) as ProductionConsumablesResponse;

      setData({
        ...productionData,
        consumables: consumablesData.items ?? [],
        consumableMovements: consumablesData.movements ?? [],
      });
    } catch (error) {
  ...
  async function decideOrder(
    ...
    try {
      ...
      setMessage({
        ...
        text:
          decision === "VALIDATE"
            ? "Production validated, consumables deducted, raw lots updated and finished stock created."
            : decision === "RETURN"
  ...
  async function adjustConsumableStock(
    consumableId: string,
    quantityDelta: number,
    comment: string,
  ): Promise<boolean> {
    ...
    try {
      const { error } = await supabase.rpc(
        "portal_adjust_production_consumable_stock",
        {
          p_token: sessionToken,
      ...
      setMessage({
        ...
        text:
          quantityDelta > 0
            ? "Consumable stock received."
            : "Consumable stock correction saved.",
      });
      await loadData();
      return true;
    } catch (error) {
  ...
  const sectionItems: Array<{
    ...
    { id: "consumables", label: "Consumable stock" },
  ...
  return (
    <div className="space-y-5">
      ...
      {section === "consumables" && (
        <ProductionConsumableStock
          profile={profile}
          data={data}
          loading={loading}
          onAdjust={adjustConsumableStock}
        />
      )}
...
function ProductSheetOperationalCard({
  ...
  return (
    <div className="border border-emerald-300 bg-emerald-50">
      ...
      {sheet.consumablesConfigured && (
        <div className="border-t border-emerald-300 bg-emerald-100 px-4 py-3 text-xs font-black uppercase text-emerald-950">
          Automatic stock · Bag {sheet.bagConsumableCode} · {sheet.consumableWastePercent}% estimated loss
          {sheet.slipSheetConsumableCode
            ? ` · Slip sheet ${sheet.slipSheetConsumableCode}`
            : " · No slip sheet for this product"}
          {" · Film and wooden pallets excluded"}
        </div>
      )}
    </div>
  );
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <section className="border border-[#cfc4b7] bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <DetailCell
            label="Bag stock reference"
            value={sheet.bagConsumableCode ?? "Not configured"}
          />
          <DetailCell
            label="Estimated consumable loss"
            value={`${sheet.consumableWastePercent}%`}
          />
...
function ProductionConsumableStock({
  profile,
  data,
  loading,
  onAdjust,
  ...
  useEffect(() => {
    if (
      ...
      !data.consumables.some((item) => item.id === selectedItemId)
    ) {
      setSelectedItemId(data.consumables[0].id);
    }
  }, [data.consumables, selectedItemId]);

  const bagsStock = data.consumables
    .filter((item) => item.consumableType === "BAG")
    .reduce((sum, item) => sum + Number(item.currentStock || 0), 0);
  const slipSheetsStock = data.consumables
    .filter((item) => item.consumableType === "SLIP_SHEET")
    .reduce((sum, item) => sum + Number(item.currentStock || 0), 0);
  ...
  async function submitAdjustment() {
    ...
    if (!selectedItemId || !Number.isFinite(delta) || delta === 0) return;
    if (!comment.trim()) return;

    const saved = await onAdjust(selectedItemId, delta, comment.trim());
    if (saved) {
      setQuantityDelta("");
      setComment("");
    }
    ...
    <div className="space-y-6">
      <section className="border border-[#2f2823] bg-[#171310] p-6 text-white">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#a89c92]">
              Bags and slip sheets are deducted only when Management validates
              the Production Order. The calculation includes 3% estimated loss.
            </p>
          </div>
          <div className="border border-[#55483e] bg-[#201a16] px-4 py-3 text-xs font-black uppercase text-[#d8cec3]">
            Film and wooden pallets excluded
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          value={formatNumber(data.consumables.length)}
        />
        <StatCard label="Bags in stock" value={formatNumber(bagsStock)} />
        <StatCard
          label="Slip sheets in stock"
          value={formatNumber(slipSheetsStock)}
        />
        <StatCard
      ...
      {data.consumables.length === 0 ? (
        <section className="border border-[#cfc4b7] bg-white">
          <EmptyState
            icon={Warehouse}
            title="No consumables configured"
            text="Run the Supabase consumable-stock migration before using this section."
          />
        </section>
      ) : (
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {data.consumables.map((item) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <span className="border border-[#d8cec3] bg-[#f4efe9] px-2 py-1 text-[10px] font-black uppercase text-[#66594f]">
                    {item.consumableType.replace("_", " ")}
                  </span>
                  <p className="mt-3 font-mono text-xs font-black text-[#b86c2c]">
                    {item.code}
                  </p>
                  <h2 className="mt-1 text-sm font-black uppercase text-[#171310]">
                    {item.description}
                  </h2>
                </div>
                <div className="text-right">
              ...
              <div className="mt-4 flex items-center justify-between border-t border-[#e4dcd3] pt-3 text-xs font-bold text-slate-500">
                <span>{item.configuredProducts} configured product(s)</span>
      ...
      {profile.role === "manager" && data.consumables.length > 0 && (
        <section className="border border-[#cfc4b7] bg-white">
          <div className="border-b border-[#d8cec3] bg-[#f4efe9] px-5 py-4">
            <p className="text-[11px] font-black uppercase tracking-[0.14em] text-[#b86c2c]">
            ...
            <p className="mt-1 text-sm font-semibold text-slate-600">
              Use a positive quantity for a receipt or initial stock, and a
              negative quantity for an inventory correction.
            </p>
          </div>
          <div className="grid gap-4 p-5 lg:grid-cols-[1fr_220px_1.5fr_auto] lg:items-end">
            ...
            <Field label="Reason / delivery reference">
              <input
                value={comment}
                onChange={(event) => setComment(event.target.value)}
                placeholder="Initial count, delivery note or correction reason"
                className={inputClass}
              />
            </Field>
            <button
              ...
              className="inline-flex h-12 items-center justify-center gap-2 bg-[#171310] px-5 text-xs font-black uppercase text-white disabled:opacity-40"
            >
              <Save size={16} />
              Save movement
            </button>
      ...
      <section className="border border-[#cfc4b7] bg-white">
        ...
        {data.consumableMovements.length === 0 ? (
          <EmptyState
            icon={History}
            title="No stock movements"
            text="Enter the initial stock or validate a Production Order to create the first movement."
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-[#201a16] text-white">
                <tr className="text-left text-[10px] font-black uppercase tracking-[0.08em]">
                  ...
                  <th className="px-4 py-4">Production basis</th>
              ...
              <tbody>
                {data.consumableMovements.map((movement) => (
                  <tr key={movement.id} className="border-b border-[#e4dcd3]">
                    <td className="whitespace-nowrap px-4 py-4 text-xs font-semibold text-slate-500">
                      {formatDateTime(movement.createdAt)}
                    </td>
                    <td className="px-4 py-4">
                      <p className="font-mono text-xs font-black">
                        {movement.consumableCode}
                      </p>
                      <p className="mt-1 max-w-xs text-xs text-slate-500">
                        {movement.consumableDescription}
                      </p>
                    </td>
                    <td className="px-4 py-4">
                      <span className="border border-[#d8cec3] bg-[#f4efe9] px-2 py-1 text-[10px] font-black uppercase">
                      ...
                      {movement.productionOrderId ? (
                        <>
                          ...
                          <p className="mt-1 text-slate-500">
                            Lot {movement.finishedErpLotNumber} · {formatNumber(movement.actualBags)} bags
                          </p>
                          <p className="mt-1 font-semibold text-[#b86c2c]">
                            Theory {formatNumber(movement.theoreticalQuantity)} + {formatNumber(movement.wastePercent)}% loss = {formatNumber(Math.abs(movement.quantityDelta))}
                          </p>
                        </>
                      ) : (
                        <span className="text-slate-400">Manual movement</span>
                      )}
                    </td>
                    <td
